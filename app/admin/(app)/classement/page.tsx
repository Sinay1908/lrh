...
const INIT = { competition: 'Nationale 1', saison: '2024-2025', position: '1', equipe: '', joues: '0', gagnes: '0', nuls: '0', perdus: '0', bpour: '0', bcontre: '0', points: '0', isLyon: false }
...
export default function ClassementPage() {
  ...
  const [filterComp, setFilterComp] = useState('Nationale 1')

  const load = useCallback(async () => {
    setLoading(true)
    try { const r = await fetch('/api/classement'); setItems(await r.json()) } finally { setLoading(false) }
  }, [])
  useEffect(() => { load() }, [load])
  ...
  const openCreate = () => { setEditing(null); setForm({ ...INIT, competition: filterComp }); setModal(true) }
  ...
  const numField = (key: string) => (
    <div style={{ fontFamily: "'Barlow Condensed',sans-serif", fontWeight: 700, textAlign: 'center' }}>
      {(filtered as unknown as Record<string,unknown>[])[0]?.[key] as number ?? '—'}
    </div>
  )
  void numField
  ...
  const cols: Col[] = [
    { label: '#', key: 'position', right: true, render: l => (
      <div style={{ width: 28, height: 28, borderRadius: 4, background: (l as Ligne).isLyon ? A.red : A.bg, color: (l as Ligne).isLyon ? '#fff' : A.textPri, display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: "'Barlow Condensed',sans-serif", fontWeight: 800, fontSize: 15, marginLeft: 'auto' }}>{l.position as number}</div>
    )},
    { label: 'Équipe', key: 'equipe', render: l => <span style={{ fontWeight: (l as Ligne).isLyon ? 700 : 500, color: (l as Ligne).isLyon ? A.red : A.textPri }}>{l.equipe as string}{(l as Ligne).isLyon ? ' ★' : ''}</span> },
    ...
    { label: 'Pts', key: 'points', right: true, render: l => <span style={{ fontFamily: "'Barlow Condensed',sans-serif", fontWeight: 800, fontSize: 16, color: A.navy }}>{l.points as number}</span> },
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: 16, flexWrap: 'wrap' }}>
          {competitions.map(c => (
            <button key={c} onClick={() => setFilterComp(c)}
              style={{ background: filterComp === c ? A.navy : A.bg, color: filterComp === c ? '#fff' : A.textSec, border: `1px solid ${filterComp === c ? A.navy : A.border}`, padding: '6px 14px', borderRadius: A.r6, cursor: 'pointer', fontFamily: "'Barlow',sans-serif", fontWeight: 500, fontSize: 13, transition: 'all 0.15s' }}>{c}</button>
          ))}
        </div>
      )}
      ...
      <Modal open={modal} onClose={() => setModal(false)} title={editing ? 'Modifier la ligne' : 'Nouvelle ligne de classement'} width={580}>
        <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr', gap: 12 }}>
          <ASelect label="Compétition" value={form.competition} onChange={e => setForm({ ...form, competition: e.target.value })}
            options={['Nationale 1','Régionale 1','Régionale 2','U17','U14','U11'].map(v=>({value:v,label:v}))} />
