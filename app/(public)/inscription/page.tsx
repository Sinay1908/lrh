...
function TarifCard({ cat, price, detail, color }: { cat: string; price: string; detail: string; color: string }) {
  const [hov, setHov] = useState(false)
  return (
    <div onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)}
      ...
      <div style={{ fontFamily: "'Barlow Condensed',sans-serif", fontWeight: 900, fontSize: 42, color, lineHeight: 1, marginBottom: 6 }}>{price}</div>
      <div style={{ fontFamily: "'Barlow Condensed',sans-serif", fontWeight: 700, fontSize: 16, color: C.navy, marginBottom: 8 }}>{cat}</div>
...
const TARIFS = [
  { cat: 'Enfant (U11)',         price: '150 €', detail: 'Saison complète · Licence FFRS incluse', color: '#1E6B9A' },
  { cat: 'Jeune (U14 / U17)',    price: '180 €', detail: 'Saison complète · Licence FFRS incluse', color: '#1E6B9A' },
  { cat: 'Senior compétition',   price: '240 €', detail: 'Saison complète · Licence FFRS incluse', color: C.navy   },
  { cat: 'Senior loisir',        price: '180 €', detail: 'Saison complète · Licence FFRS incluse', color: '#2A7A4B' },
  { cat: 'Famille (2+ membres)', price: '–10%',  detail: 'Réduction sur chaque licence supplémentaire', color: C.red },
]
      ...
      <div style={{ background: C.lightBluePale, padding: SECTION_PAD }}>
        <div style={{ ...MAX_W }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(185px,1fr))', gap: 14, maxWidth: 960, margin: '0 auto 24px' }}>
            {TARIFS.map(t => <TarifCard key={t.cat} cat={t.cat} price={t.price} detail={t.detail} color={t.color} />)}
          </div>
